...
Tài liệu này cung cấp các hướng dẫn cơ bản để làm việc với thư viện `@namphuong/acs-chat-react`.
...
2. Mở terminal tại thư mục **dự án cần test**, chạy lệnh install trỏ tới file `.tgz` vừa tạo:
   ...
   ```
   *(Đường dẫn có thể là đường dẫn tuyệt đối hoặc tương đối từ dự án cần test)*.
...
2. Tại thư mục **dự án cần test**:
   ```bash
   npm link @namphuong/acs-chat-react
   ```
*(Lưu ý quan trọng: Vì tính chất của symlink, npm link thường gây lỗi trùng lặp React (`Invalid hook call`) nếu cả thư viện và dự án test đều có `node_modules/react`. Nếu gặp lỗi này, tốt nhất hãy chuyển sang cách dùng `npm pack` ở trên).*
...
npm run build
```
*(Lưu ý: Lệnh này sẽ chạy `tsc --noEmit && vite build`, đảm bảo xuất ra đúng định dạng CommonJS (.cjs), ESModules (.js), và type definitions (.d.ts) như đã cấu hình trong `package.json`).*
